...
if (!empty($titlesAndAppointments)) {
    ...
    foreach ($titlesAndAppointments as $item) {
        ...
        if (!empty($item['Revision_document']) && $item['Revision_Upload_button'] == 'Yes') {
            echo "<form style='margin-top:-20px; left:-100px' method='POST' action='admin_upload_revision.php' enctype='multipart/form-data'>
    ...
    function rescheduleAppointment(newDate) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch('admin-researcherRescheduleAppointment.php', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire('Rescheduled!', 'Your appointment has been rescheduled.', 'success').then(() => {
                                ...
                                window.location.href = 'admin-researcherHome.php';
