  ...
  <div class="menu-btn">
    <div class="navigation">
      ...
      <a href="admin_activateSuperuser.php">Super Users</a>
        ...
        <!-- Logout Button -->
        <form method="POST" action="admin_rev_stats.php" style="display: inline;">
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . htmlspecialchars($row['appointment_date']) . "</td>"; // Display appointment date or "No Schedule"
...
document.addEventListener('DOMContentLoaded', function () {
   ...
   function filterTable() {
       ...
       for (let row of rows) {
           const titleCell = row.cells[0]; // Study Protocol Title
           const emailCell = row.cells[5]; // Email
           const categoryCell = row.cells[1]; // Research Category
           const collegeCell = row.cells[2]; // College
           const dateCell = row.cells[4]; // Assuming the date is in the first column, adjust if necessary
           ...
           if (matchesSearchTitle && matchesSearchEmail && matchesCategory && matchesCollege && matchesDate) {
               row.style.display = '';
               updateRowCount()
   ...
   function filterByReviewer() {
       ...
       xhr.open('POST', 'fetch_rti_by_reviewer_notdone.php', true);
