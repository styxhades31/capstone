...
$query = "
  ...
  AND ar.researcher_info_id IS NULL   -- Exclude rows where rti.id exists in assign_reviewer
...
$query1 = "
SELECT 
    rti.id,  -- Add the id field here
    rti.uploaded_at,
    GROUP_CONCAT(
        CONCAT(ri.first_name, ' ', ri.last_name, 
               IF(ri.suffix IS NOT NULL, CONCAT(' ', ri.suffix), '')
        ) SEPARATOR ', '
    ) AS researchers_involved,
    rti.study_protocol_title,
    rti.research_category,
    rti.college,
    rti.adviser_name,
    ...
    rti.type_of_review
FROM 
    researchertitleinfo_nouser AS rti
LEFT JOIN 
    researcherinvolved_nouser AS ri 
ON 
    rti.id = ri.researcher_title_id
GROUP BY 
    rti.id
ORDER BY 
    rti.uploaded_at DESC;
";
...
<!-- admin_rev_sats.php-->
<a href="admin_rev_stats.php">
<button class="reviewedbtn">View Assigned titles</button>
</a>


<a href="admin_reviewed_titles.php">
<button class="reviewedbtn1">View Reviewed titles</button>
</a>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . ($row['appointment_date'] ? htmlspecialchars($row['appointment_date']) : "No Schedule") . "</td>"; // Display appointment date or "No Schedule"
        ...
        echo "<td>
              ...
              echo "<td><button class='assign-btn' data-id='" . $row['id'] . "'>Assign</button></td>"; // Use id field for data-id
    ...
    echo "<tr><td colspan='11'>No data available.</td></tr>"; // Adjust colspan based on the number of columns
}
?>
 </tbody>
            </table>
  ...
  <!-- partial -->
    ...
    function filterTable() {
        for (let row of rows) {
            const titleCell = row.cells[1]; // Study Protocol Title (index 3)
            const emailCell = row.cells[6]; // Email (index 9)
            const categoryCell = row.cells[2]; // Research Category (index 4)
            const collegeCell = row.cells[3]; // College (index 5)
            const paymentCell = row.cells[8]; // Payment (index 11)
            const typeCell = row.cells[9]; // Type of Review (index 12)
            ...
            const paymentValue = paymentCell ? paymentCell.querySelector('select').value.toLowerCase() : '';
            const typeValue = typeCell ? typeCell.querySelector('select').value.toLowerCase() : '';
            ...
            if (matchesSearchTitle && matchesSearchEmail && matchesCategory && matchesCollege && matchesPayment && matchesType) {
                row.style.display = '';
                 updateRowCount()
            } else {
