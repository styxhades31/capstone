        ...
        if ($_SESSION['failed_attempts'] >= 5) {
            ...
            if ($result && $result->num_rows > 0) {
                ...
                if ($row['verification_code'] === $verificationCode) {
                   ...
                   // After successfully updating the user record and setting email and verification_code to NULL
    ...
    if ($userIdResult && $userIdResult->num_rows > 0) {
        ...
        if ($roleIdResult && $roleIdResult->num_rows > 0) {
            ...
            if ($insertRoleStmt->execute()) {
                // Insert into reviewer_profiles table
                $firstName = $_SESSION['first_name'];
                $lastName = $_SESSION['last_name'];
                $middleInitial = $_SESSION['middle_initial'];


                $insertProfileStmt = $conn->prepare("INSERT INTO reviewer_profiles (user_id, first_name, last_name, middle_initial) VALUES (?, ?, ?, ?)");
                $insertProfileStmt->bind_param("isss", $userId, $firstName, $lastName, $middleInitial);

                if ($insertProfileStmt->execute()) {
                    $success = 'Account created successfully! Wait for the account activation.';
                    // Unset session variables
                    unset($_SESSION['email'], $_SESSION['password'], $_SESSION['role'], $_SESSION['first_name'], $_SESSION['last_name'], $_SESSION['middle_initial']);
                } else {
                    $error = 'Error creating reviewer profile. Please try again.';
                }
            } else {
...
<html lang="en">
<head>
	<title>Reviwer Account Verification</title>
	<meta charset="UTF-8">
...
<div class="container">
		...
		<div class="container-login100">
			<div class="wrap-login100">
				<div class="login100-form-title" style="background-image: url(./img/wmsu5.jpg);">
					...
					<h4 class="sign">ACOUNT VERIFICATION </h4>
				</div>
    ...
    <div class="wrap-input100 validate-input m-b-26" data-validate="Code is required">
        ...
        <input class="input100" type="text" id="verificationCode" name="verificationCode" required placeholder="Enter Code">
